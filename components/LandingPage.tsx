...
const FeatureCard: React.FC<{ icon: string; title: string; description: string }> = ({ icon, title, description }) => (
  <div className="flex flex-col items-center text-center p-6 bg-slate-100/50 dark:bg-slate-800/50 rounded-xl backdrop-blur-sm">
...
const LandingPage: React.FC<LandingPageProps> = ({ onStart }) => {
  ...
  return (
    <div className="w-full relative py-8 sm:py-12">
      <div
        className="absolute inset-0 z-0 bg-no-repeat bg-center bg-cover opacity-[.03] dark:opacity-[.05] pointer-events-none"
        style={{
      ...
      <div className="relative z-10">
        <div className="max-w-4xl mx-auto text-center py-12 sm:py-20 px-4">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900 dark:text-slate-50 mb-4 tracking-tight">
            {t('landing-title')}
          </h1>
          <p className="max-w-2xl mx-auto text-base sm:text-lg text-slate-600 dark:text-slate-300 mb-8">
            {t('landing-subtitle')}
          </p>
          <button
            onClick={onStart}
